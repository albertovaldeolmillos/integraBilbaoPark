...
namespace Codaxy.WkHtmlToPdf
{
    ...
    public class PdfConvert
    {
        ...
        private static string GetWkhtmlToPdfExeLocation()
        {
            ...
            filePath = oUrlHelper.RequestContext.HttpContext.Server.MapPath(Path.Combine("~/App_Data", @"wkhtmltopdf\wkhtmltopdf.exe"));
            if (File.Exists(filePath))
                return filePath;

            customPath = ConfigurationManager.AppSettings["wkhtmltopdf:path"];
